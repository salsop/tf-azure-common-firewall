...
resource "azurerm_storage_share_directory" "plugins" {
  ...
  provisioner "local-exec" {
    when       = destroy
    command    = "az storage file delete-batch --account-name ${self.storage_account_name}  --source bootstrap/plugins"
    on_failure = continue
  }
...
resource "azurerm_storage_share_directory" "software" {
  ...
  provisioner "local-exec" {
    when       = destroy
    command    = "az storage file delete-batch --account-name ${self.storage_account_name} --source bootstrap/software"
    on_failure = continue
  }
...
resource "azurerm_storage_share_directory" "license" {
  ...
  provisioner "local-exec" {
    when       = destroy
    command    = "az storage file delete-batch --account-name ${self.storage_account_name} --source bootstrap/license"
    on_failure = continue
  }
...
resource "azurerm_storage_share_directory" "config" {
  ...
  provisioner "local-exec" {
    command = "az storage file upload-batch --account-name ${azurerm_storage_account.bootstrap.name} --account-key ${azurerm_storage_account.bootstrap.primary_access_key} --destination ${azurerm_storage_share.bootstrap.name}/config  --source tmp/config"
  }

  provisioner "local-exec" {
    when       = destroy
    command    = "az storage file delete-batch --account-name ${self.storage_account_name} --source bootstrap/config"
    on_failure = continue
  }
...
resource "null_resource" "license" {
  provisioner "local-exec" {
    command = "az storage file upload-batch --account-name ${azurerm_storage_account.bootstrap.name} --account-key ${azurerm_storage_account.bootstrap.primary_access_key} --destination ${azurerm_storage_share.bootstrap.name}/license  --source tmp/license"
  }
...
data "template_file" "authcodes" {
  template = file("template_files/authcodes.template")
  vars = {
...
data "template_file" "initcfg_txt" {
  template = file("template_files/init-cfg.txt.template")
  vars = {
...
resource "local_file" "initcfg_txt" {
  filename = "${path.module}/tmp/config/init-cfg.txt"
